defmodule HandrollWeb.AccountAuthTest do
  use HandrollWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Handroll.Accounts
  alias Handroll.Accounts.Scope
  alias HandrollWeb.AccountAuth

  import Handroll.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, HandrollWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{account: %{account_fixture() | authenticated_at: DateTime.utc_now()}, conn: conn}
  end
  ...
  describe "logout_account/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      HandrollWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        HandrollWeb.Endpoint.subscribe("accounts_sessions:#{Base.url_encode64(token)}")
      end
