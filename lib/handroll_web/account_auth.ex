defmodule HandrollWeb.AccountAuth do
  use HandrollWeb, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias Handroll.Accounts
  alias Handroll.Accounts.Scope

  # Make the remember me cookie valid for 14 days. This should match
  # the session validity setting in AccountToken.
  @max_cookie_age_in_days 14
  @remember_me_cookie "_handroll_web_account_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_account(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      HandrollWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      HandrollWeb.Endpoint.broadcast(account_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule HandrollWeb.PageLive do
        use HandrollWeb, :live_view

        on_mount {HandrollWeb.AccountAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{HandrollWeb.AccountAuth, :require_authenticated}] do
  ...
  def on_mount(:require_sudo_mode, _params, session, socket) do
    ...
    if Accounts.sudo_mode?(socket.assigns.current_scope.account, -10) do
      {:cont, socket}
    else
      socket =
        socket
