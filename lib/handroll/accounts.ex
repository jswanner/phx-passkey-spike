defmodule Handroll.Accounts do
  @moduledoc """
  ...
  See `Handroll.Accounts.Account.email_changeset/3` for a list of supported options.
  ...
  See `Handroll.Accounts.Account.password_changeset/3` for a list of supported options.
  ...
  Returns the updated account, as well as a list of expired tokens.
  ...
  def update_account_password(account, attrs) do
    ...
    |> case do
      {:ok, account, expired_tokens} -> {:ok, account, expired_tokens}
  ...
  defp update_account_and_delete_all_tokens(changeset) do
    %{data: %Account{} = account} = changeset
